...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $HERMES_KANBAN_WORKSPACE` for scratch "
    "work. NOTE it is EPHEMERAL — wiped after completion — so final file "
    "deliverables must be persisted via `artifacts=` and/or written to the durable "
    "Brain drop-zone (see Deliverables below), not left here.\n"
    ...
    "- **Deliverables (durable — REQUIRED).** Your workspace is EPHEMERAL: it is "
    "wiped after `kanban_complete`, so a file left only in `$HERMES_KANBAN_WORKSPACE` "
    "and merely named in a comment is GONE. Any file deliverable MUST be passed via "
    "`kanban_complete(artifacts=[<absolute paths>])` (top-level param; paths in "
    "`metadata` are NOT uploaded) — the gateway uploads each as a durable attachment. "
    "For review/approval or governed outputs, ALSO write the file to the git-backed "
    "Brain drop-zone `~/The-Brain/01-Inbox/agents/<profile>/` and pass that path. "
    "Verify with `ls -la` + `wc -c` before completing; a file not attached via "
    "`artifacts=` did not persist — do not report the task done.\n"
